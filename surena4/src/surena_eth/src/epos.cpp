...
void Epos::DataReceived(QByteArray data)
{
   ...
   // qDebug()<<QString ::number( bumpPacket->ID&0xffff,16)<<" ? "<<mid.toHex();

    if ( (data[0]&0xff)== 0x01 && (data[1]&0xff)== 0x55 && (data[2]&0xff)== 0xAA && (data[3]&0xff)== 0x55 )
    {
        //--------------------
        for(int i=0;i<16;i++){
    positionsInc.append(incommingPacket->MotorData[i].Valu1);
    positions.append(incommingPacket->MotorData[i].Valu2);
        }
      ...
      //-------
        for(int i=0;i<12;i++){
          ft.append(incommingPacket->FTsensor[i]);
           }
        //--------------------
